...
function updateObjectStart(objectId, objectType) {
  ...
  const cardButton = document.getElementById(
    `${objectType}EditButton-${objectId}`
  );
...
function createTable() {
  let containerElements = currentStorage.map((container) => {
    ...
    return `<div class="card box-storage" id="container-${container.id}">
        ...
        <form action="/client/update-box/${
          roomId.innerHTML
        }" style="display: none;" id="boxUpdate-${container.id}" method="POST">
              <div class="form-group">
                <input type="text" class="form-control" id="formGroupExampleInput" name="name"
                  value="${container.box}">
              </div>
              <button  id="boxEditConfirm-${container.id}" name="id" value="${
      container.id
    }" type="submit"
              class="btn content-btn">
                Confirm</button>
        ...
        <button id="boxEditButton-${container.id}" onclick="updateObjectStart(${
      container.id
    }, 'box')" class="btn content-btn card-link">Edit Box</button>
        <a class="btn content-btn" href="/object-handling/delete/box/room-view/${
          container.id
        }/?roomId=${roomId.innerHTML}">Delete Box</a>

        <form action="/object-handling/create-item/${roomId.innerHTML}/${
      container.id
    }">
            <button class="btn content-btn add-btn" name="containerName" value="${
              container.box
            }">Add New Item</button>
        </form>
        <div class="box-drag-position" id="drag${
          container.id
